...
int f_minus_update(map<Point_2,int>& numberOfG, set<Point_2>& CoveredP,int i ) // f( indeksSet U {i} )  - f( indeksSet) 
{
     int difference = 0;
     for(Point_2 p : S[i])
     {
         if( !( CoveredP.count( p ) > 0))          // !( CoveredPoints.count( p ) > 0)) //pointExists.find(p) == pointExists.end() )
         {
             difference++;
         }
     }
...
**/
float greedy_criterion(vector<int>& C,map<Point_2,int>& numberOfG, set<Point_2>& CoveredP, int i) // take s_i from S
{   //cout << "greedy_criterion " << endl;

    int f_m =  f_minus_update(numberOfG, CoveredP, i );
    if(f_m == 0)
       return INFEASIBLE;
...
**/
float gridi_criterion_dragan(vector<int> &index,map<Point_2,int>& numberOfG, set<Point_2>& CoveredP, int i)
{
     ...
     /* if(index.size() > 0 and std::find( index.begin(), index.end(), i)  != index.end() )
      ...
      int number_after_add_i = f_minus_update(numberOfG, CoveredP, i ); 
      int correct_total =  CoveredP.size() +  number_after_add_i;//cardinality_by_index(index);  // broj pokrivenih tacaka diskretizacije
...
int min_greedy(vector<int>& indeks1,map<Point_2,int>& numberOfG, set<Point_2>& CoveredP)
{        
           
          float g_m = INFEASIBLE; 
          float g_mi;
          int dodaj = -1; 
		  //cout << " isize " << indeks1.size() << " " << S.size() <<  endl;
          for(int i = 0; i < S.size(); ++i)
          {    //cout <<"i: " << i << " ";
              ...
              {  //cout << " nalazi se " << i << "\n "; 
                 ...
                 switch(greedy){
                        ...
                        case 0 :  g_mi = greedy_criterion(indeks1,numberOfG, CoveredP, i); break;
                        case 1 :  g_mi = greedy_criterion_1(i); break;
                        case 2 :  g_mi = greedy_criterion_2(indeks1, i) ; break;
                        default: g_mi = gridi_criterion_dragan(indeks1,numberOfG, CoveredP, i);
                 }
                 bool us = g_mi <= g_m; 
         ...
         /* if(dodaj == -1){
          	cout<<"Evo ga"<<endl;
          	cout<<"g_mi: "<<g_mi<<endl;
          	cout<<"indeks.size: "<<indeks1.size()<<endl;
          	cin.get();
		  }*/
          return dodaj;
}
		   ...
		   while(shak && k < copy.size()/3)
		   {
		   	...
		   	//	cout<<"izbacujemo:"<< k<<" elemenata."<<endl;
				...
				while(i < k){
			           ...
			           //cout<<"popravka u shakingu prije min_greedy" <<  endl;
					   int index_set = min_greedy(copy,numberOfGuards2,CoveredPoints2); 
           			   //cout<<"popravka u shakingu poslije min_greedy" <<  endl;
           			   //if(!findA(copy, index_set)){ //jos nije dodan
                                   if( index_set == -1) 
                                      {
                                      	cout << "Covered copy=" << CoveredPoints2.size() <<"Potrebno"<<f_S<< endl;
										  //cin.get();
										  force = true;
										  break;
