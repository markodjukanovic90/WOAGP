...
where $ (u_{x},u_{y}),(v_{x},v_{y})$ are adjacent vertices of polygon $P$.
	...
	Treba primjetiti da iz pokrivenosti diskretizacije D(P) skupom stražara iz Z ne slijedi pokrivenost poligona P istim skupom stražara Z. Na slici 1.1 pod a) prikazan je poligon koji nije pokrivan, dok je njegova diskretizacija pokrivena. Trokut na dnu poligona predstavlja nepokrivenu regiju. Poligon na slici 1.1 pod b) je pokriven kao i njegova diskretizacija.
	...
	Couto et al.~\cite{couto2007exact} koristili su pomenutu diskretizaciju za problem ortogonalne galerije. U njihovom radu broj pologona nad kojim je primjenjena ova diskretizacija je 10282. Za 59.4\% poligona je vazilo da iz pokrivenoosti diskretnog skupa D(P) slidi pokrivensort poligona P za optimalno pronadjen skup strazara koji pokrivaju D(P). Oni su dodavali nove tačke u D(P) iz nepokrivenih regija kako bi iz pokrivenosti diskretizacije slijedila pokrivenost poligona za sve poligone. Nad testiranim uzorkom se pokazalo da se broj novododanih tačaka može opisati regresivnom funkcijom  0.003·n+1.1, gjde je n broj tjemena poligona. Dakle, ili je čitav poligon pokriven ili mali broj regija je nepokriveno optimalnim skupom stražara Z za D(P). U sekciji 4 će biti prikazana površina ovih nepokrivenih regija.
...
\subsubsection{A Hybrid of the \textsc{Greedy + \textsc{Cplex}}}
 ...
 We used two different kind of benchmark sets from~\cite{tomas2006visibility}:
   ...
   \end{table}
     ...
     \end{itemize}
    \fxnote{Milan: generisati vrijeme izvrsavanja...--- Kog izvrsavanja?}
    
    \begin{figure}
    	\centering
    	\includegraphics[width=\textwidth]{grafikon1.eps}
    	\caption{Polygon Coverage}
    	\label{fig:cov}
    \end{figure}
