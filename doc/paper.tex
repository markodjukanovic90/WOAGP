...
\begin{equation}\label{eq:sp}
	...
	\begin{itemize}
		...
		\item structure \texttt{CoveredPoints} -- which keeps a set of points from $D(P)$ that are covered by the partial solution;
		\item function \texttt{updateCoveredPointsAdd (vertex v)} -- does the update of  structures \texttt{CoveredPoints} and \texttt{numberOfGuards} by considering all new points from $D(P)$ covered by vertex $v$, when $v$  is added to partial solution; %pseudocode of the procedure is shown in Algorithm \ref{alg:updateCoveredPointsAdd}.
