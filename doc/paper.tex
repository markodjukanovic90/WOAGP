...
\begin{document}
	...
	\begin{itemize}
		...
		\item In a comprehensive computational experiment, \fxnote{TODO: popraviti} different variants of approximative algorithms we tested, analysed, and their efficiency checked. The methods are then compared to the exact approaches ILP and \emph{Constraint Programming} (CP) w.r.t. the quality of obtained heuristic solutions as well as runtimes.
	\end{itemize}
...
where $ (u_{x},u_{y}),(v_{x},v_{y})$ are adjacent vertices of polygon $P$.
	...
	q_1(s^p, p_i) = \frac{w_{p_i}}{ f(s^p \cup \{p_i\})  - f(s^p)},
	\end{align}
	...
	\begin{itemize}
		...
		\item function \texttt{updateCoveredPointsAdd (vertex v)} -- does the update of  structures \texttt{CoveredPoints} and \texttt{numberOfGuards} by considering all new points which are covered by vertex \texttt{v}, when \texttt{v} is added to solution; pseudocode of the procedure is shown in Algorithm \ref{alg:updateCoveredPointsAdd}. Its time complexity is $O(n)$.  
