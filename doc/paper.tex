...
\subsection{A Hybrid of the \textsc{Greedy} and \textsc{Cplex}}
 ...
 We used two different kind of benchmark sets from~\cite{tomas2006visibility,bajuelos2004partitioning}:
     \begin{itemize}
     	 \item \emph{MinArea} instances: include polygons with a small areas and tiny interior. They present a lower boundary case for the cardinality of set $D(P)$ which means that, beside the polygon vertices,  $D(P)$ contains few additional points.
...
w_i := \frac{l_i + l_{i+1}}{2}.
 ...
 This way of assigning weights is augmented by the fact that if the arithmetic length of both edges that comes out of vertex $i$ is longer, it is expected that a guard can see a larger pieces of polygon $P$. This implies that the spectrum of camera installed at $v_i$ has to be larger, which again implies that it should be of a higher quality, i.e,. a higher price.
		\item \emph{point-based related weights} ($W1$): For each vertex $v_i$, based on the number of points in $D(P)$ that are visible from $v_i$, we assign prices to each vertex on the following way:
		\begin{equation}
   ...
   \end{table}
    ...
    \begin{figure}[!ht]
    	...
    	\caption{Avg. execution time of the compared algorithms. \fxnote{ujednacinit font teksta u legendi sa fontom u tekstu, dodati $[s]$ na $y$ osi, nigdje ne referenciramo ovu sliku...}}
    	\label{fig:timealg}
    \end{figure}
