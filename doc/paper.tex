...
$ \bigtriangleup_{y}=min\{ |u_{y}-v_{y}|,\ u_{y}\neq v_{y}\ |\  u,v\ adjacent\ vertices\ of\ the\ polygon\ P \}$. \fxnote{tekst ne bi trebao biti italic, koristiti mbox naredbu ili mathrm, min je naredba u latexu  "min"}
	...
	The intersection points of this grid that are interior to $P$ and all vertices from $P$ are added to the set $D(P)$.\fxnote{Ova recenica je nejasna, moze li malo bolji prevod?} The pseudocode for building the discretizations $D(P)$ is given in Algorithm~\ref{alg:discret}.
	
	\fxnote{TODO: zasto uzimamo ovakav tip diskretizacije, obajsniti zasto id pokrivenosti diskretizacije ne slijedu pokrivenost poligona -- dvije slike}
