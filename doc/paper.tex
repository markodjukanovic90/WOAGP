...
\begin{document}
	...
	\title{Performance evaluation of two greedy-like heuristics for solving  the Weighted Orthogonal Art Gallery Problem under regular grid discretization}
...
\author[1]{Dragan Mati\'c}
	...
	\begin{itemize}
		...
		\item Both an existing and the novel greedy algorithms are hybridized with the ILP.
		...
		\item In a comprehensive computational experiment,  we tested, analysed and check the efficiency of different variants of the considered algorithms. The methods are then compared to the exact approaches ILP and \emph{Constraint Programming} (CP) w.r.t. the quality of obtained heuristic solutions as well as runtimes.
	\end{itemize}
...
where $ (u_{x},u_{y}),(v_{x},v_{y})$ are adjacent vertices of polygon $P$.
	...
	g_1(s^p, p_i) = \frac{w_{p_i}}{ f(s^p \cup \{p_i\})  - f(s^p)},
	\end{align}
	where 
\begin{equation}\label{eq:sp}
    f(s^p) = |\bigcup_{s \in s^p} s |.
    \end{equation}
	...
	In this subsection we present a novel greedy function, which is used in the greedy algorithm. First, we introduce a term  ``incorrect point''; for a point from $D(P)$ we say that it is incorrect if it is not covered by any guard from a current partial solution $s^{ps}$. Let us denote by $incorrect_{total}(s^{sp})$ the total number of incorrect points from discretization $D(P)$ for the current partial solution. Note that  $incorrect_{total}(s^{ps}) = |D(P)|-f(s^{ps})$, where $f(s^{ps})$ is defined in (\ref{eq:sp}) and $|D(P)|$ is hte cardinality of discretization set . Let $w_{total}$ be the total sum of all weights among all vertices. The greedy function for the given partial solution $s^{ps}$ and the guard $v$ takes into account both the value of the objective function of the WOAGP  applied to the partial solution when guard $v$ is included in $s^{ps}$ and the total number of incorrect points, given by the formula:
	...
	In the Equation  (\ref{eq:greedyfun2}) both terms are normalized in order to better balance trade off between two factors which influence on the search. The algorithm does not  ultimately prefer any of criteria for choosing next vertex. Although it cannot be a rule, it is justified to suppose that the second term has a more influence on the choice of the next vertex in earlier phases of the algorithm execution. At the beginning, more points are uncovered and the algorithm will choose such vertices which cover larger area of the polygon. At the end of algorithm, it could be expected that the first term plays more significant role, since the total number of uncovered points is small.
	...
	In our experimental studies, we found out that this heuristic does not perform well on its own, but presents a reasonable tie-breaking mechanism and is able to boost quality of our greedy heuristics which includes criterion (\ref{eq:greedyfun2}).
	\subsubsection{Partial calculation of greedy functions}
	
	In order to enable fast calculation of greedy functions described in previous subsections, we noticed that it could be useful to allow fast updating of the number of uncovered vertices. Therefore, we introduced several useful structures and auxiliary functions:
	\begin{itemize}
		...
		\item function \texttt{updateCoveredPointsAdd (vertex v)} -- does the update of  structures \texttt{CoveredPoints} and \texttt{numberOfGuards} by considering all new points which are covered by vertex $v$, when $v$  is added to solution; pseudocode of the procedure is shown in Algorithm \ref{alg:updateCoveredPointsAdd}. 
		\item function \texttt{updateCoveredPointsRemove(vertex v)} -- updates  structures \texttt{CoveredPoints} and \texttt{numberOfGuards} by removing all points which are covered by $v$ , when $v$ is removed from partial solution; pseudocode  is shown in Algorithm \ref{alg:updateCoveredPointsRemove}.
	\end{itemize}
 Although the time complexity of both functions is $O(|D(P)|)$, from Algorithms \ref{alg:updateCoveredPointsAdd} and \ref{alg:updateCoveredPointsRemove} it can be seen that they only depends on the number of points covered by the current vertex, which is expected to be significantly less than the upper bound. These two functions allow fast calculation of the functions $f$ and  $incorrect_{total}$: a candidate vertex $v$ is temporary added to the partial solution, new status is checked and the vertex is then removed from the solution.
	\begin{algorithm}[!t]
          ...
          %\begin{comment}
            ...
            \begin{algorithm}[!t]
          	\caption{Function updateCoveredPointsRemove}\label{alg:updateCoveredPointsRemove}
          	\begin{algorithmic}[1]
          		\State \textbf{Input:} vertex $v$, $s[v]$:  list of points covered by $v$
          		\State \textbf{Output:} updated structures coveredPoints and numberOfGuards
          		\For{$p \in S[v]$}
          ...
          \end{algorithm}
	 % \end{comment}
\subsubsection{A Hybrid of the \textsc{Greedy + Cplex}}
	...
	The instances of the specific OAGP with up to a 200 vertices are used in our experiments to which a weight to each vertex of polygons is assigned. We included two kind of weights into these benchmarks:
	\begin{itemize}
	...
	\end{itemize}
     ...
     \end{itemize}
     For small and large instances, one polygon for each $n \in\{8,10,...,200\}$ has been generated, which makes 96 instances per each benchmark set. In overall, we have 192 polygons which vertices with two different weights, which gives us, in overall, 384 problem instances.
     \subsection{Settings and the choice of the Parameters}
      ...
      \begin{itemize}
        \item \textsc{Greedy + Cplex} parameters: $K = 0.05 \cdot n$.
      \end{itemize}
	...
	\subsection{Results and Discussion}
        ...
        \end{itemize}  and 6 heuristic approaches:
           \begin{itemize}
           	\item three pure greedy methods, guided by $g_1$, $g_2$ and $g_3$ greedy criterion. We label them by \textsc{Greedy-1}, \textsc{Greedy-2} and \textsc{Greedy-3}, respectively;
            ...
            \item three \textsc{Greedy+Cplex} variants, henceforth labeled by  \textsc{Greedy}-$i$+\textsc{Cplex}, $i=1,2,3$,
            where $i$ stands for heuristic $g_i$ involved as the greedy criterion of the hybrid;
           \end{itemize}
         From our results, we noticed that there is no significant difference between the results of \textsc{Greedy-2} and \textsc{Greedy-3}. Therefore, we report only the results of \textsc{Greedy-2} algorithm.
         Summarized numerical results for all of our algorithm variants are displayed in Table~\ref{tab: t_w0} and Table~\ref{tab: t_w1} for topollogically-based benchmarks and point-based benchmarks, respectively. Each of the tables is additionally  subdivided by a vertical line into two parts, depending on the case if we consider \emph{small} or \emph{large} polygons for a specific weight.
         For each of the algorithm  we reported average quality and average standard deviation ($\sigma$) over all instances (96 instances) of the considered subgroup. If an asterisk is given, i means taht optimality is proven for all instances of considered subgroup.
         \begin{table}
         \centering
         \begin{tabular}{ |p{4cm}|p{1.5cm}p{1cm}|p{1.5cm} p{1cm} |  }
         	\hline
         	\multicolumn{5}{|c|}{Avg. results on the topollogically-based benchmarks.} \\
         	\hline
            \multicolumn{1}{|c|}{}&
            \multicolumn{2}{|c|}{$Small$}&
            \multicolumn{2}{|c|}{$Large$} \\
            \hline	
         	Algorithm  & $|\bar{s}|$ & $\bar{\sigma}$ & $|\bar{s}|$ & $\bar{\sigma}$  \\ \hline \hline
            \textsc{Cplex} & 15.93* & 8.12 & 2.62* & 1.38 \\ \hline
            \textsc{Cp} &  16.04 & 8.24 &  2.62*  & 1.38\\ \hline
         	\textsc{Greeedy-1} & 22.04  & 12.02 & 3.38   & 1.59 \\ \hline
         	\textsc{Greeedy-2} & 19.97  & 9.60  &  3.51 & 1.80\\ \hline
         	\textsc{Greeedy-1 + Cplex}  & 18.49 &  9.56  & 3.34 & 1.41 \\ \hline
         	\textsc{Greeedy-2 + Cplex}  & 18.39 & 8.96 &  3.32 & 1.29 \\ \hline

         	\hline
         \end{tabular}
         \caption{Results of different combinations of the algorithms.} \label{tab: t_w0}
        \end{table}
       ...
       \begin{table}
    	...
    	\begin{tabular}{ |p{4cm}|p{1.5cm}p{1cm}|p{1.5cm} p{1cm} |  }
    		\hline
    		\multicolumn{5}{|c|}{Avg. results on the point-based benchmarks.} \\
    		\hline
    		\multicolumn{1}{|c|}{}&
    		\multicolumn{2}{|c|}{$Small$}&
    		\multicolumn{2}{|c|}{$Large$} \\
    		\hline	
    		Algorithm  & $|\bar{s}|$ & $\bar{\sigma}$ & $|\bar{s}|$ & $\bar{\sigma}$  \\ \hline \hline
    		\textsc{Cplex} & 36.03* & 19.04 & 6.72* & 3.07 \\ \hline
    		\textsc{Cp} & 36.03* & 19.04 & 6.72* &  3.07 \\ \hline
    		\textsc{Greeedy-1} &39.28  & 19.55 & 6.76 & 3.06 \\ \hline
    		\textsc{Greeedy-2} & 40.92 & 19.54 & 6.76 & 3.06 \\ \hline
    		\textsc{Greeedy-1 + Cplex} & 37.78 & 19.59 &  6.72 & 3.07 \\ \hline
    		\textsc{Greeedy-2 + Cplex} & 37.31 & 19.13 &  6.72 & 3.07 \\ \hline
    	
    		\hline
    	\end{tabular}
    	\caption{Result of different combinations of the algorithms.} \label{tab: t_w1}
    \end{table}
        From the numerical results, we observe the following conclusions:
        \begin{itemize}
        	\item  For the topollogically--based benchmarks and the exact approaches, \textsc{Cplex} achieved best results among other approaches. moreover, Cplex could prove optimallity of all (192) instances, while \textsc{Cp} could not. It is expecially noticed for small polygons, where it seems hard for \textsc{Cp} to prove optimality.
        	\item For the point--based benchmark set and the exact approaches, \textsc{Cplex} again show its power, since it could prove all instances to optimallity, which is also the case of \textsc{Cp}.
        	\item  For the topollogically--based benchmarks and the heuristic approaches\fxnote{TODO...}
        \end{itemize}
          \fxnote{TODO: generisati plotove za vremena svakog od algoritama (da se vidi da je greedy brzi -- nadamo se)}
