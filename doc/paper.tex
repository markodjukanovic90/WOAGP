...
$ \bigtriangleup_{y}=min\{ |u_{y}-v_{y}|,\ u_{y}\neq v_{y}\ |\  u,v\ adjacent\ vertices\ of\ the\ polygon\ P \}$. \fxnote{tekst ne bi trebao biti italic, koristiti mbox naredbu ili mathrm, min je naredba u latexu  "min", mjesto $"|"$ korisititi naredbu "mid"; koristis nedefinisane simbole $u_x$ i $u_y$ }
	...
	The intersection points of this grid that are interior to $P$ and all vertices from $P$ are added to the set $D(P)$.\fxnote{Ova recenica je nejasna, moze li malo bolji prevod? Referisi u odnosu na Pseudokod} The pseudocode for building the discretizations $D(P)$ is given in Algorithm~\ref{alg:discret}.
	
	\fxnote{TODO: zasto uzimamo ovakav tip diskretizacije, objasniti zasto id pokrivenosti diskretizacije ne slijedu pokrivenost poligona -- dvije slike}
	...
	\begin{itemize}
		...
		\end{equation}
	\end{itemize}
     We used three different kind of benchmarks:
     \begin{itemize}
