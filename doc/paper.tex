...
The ILP  model for the MWSCP ~\cite{vazirani2013approximation}   is  adapted to WOAGP  as follows:
	...
	In this section we present the heuristic greedy approaches for solving the considered problem and their hybridization with the exact ILP method.
 ...
 A general pseudocode of Greedy heuristics is given in Algorithm~\ref{alg:greedy}. 	The set Extend($s^{ps}$) includes those guards from set $V \setminus$ $s^{ps}$ that cover some points from $D(P)$ that are not yet covered by $s^{ps}$. Solution component of the problem is a guard. Extension of partial solution $s^{ps}$ by a vertex $v$ corresponds to adding $v$ into set $s^{ps}$.
	...
	Concerning the greedy heuristic for the literature for solving MWSCP ~\cite{chvatal1979greedy, lovasz1975ratio}, one of the most efficient greedy heuristics was based on the following criterion:
...
\subsection{A Hybrid of the \textsc{Greedy} and \textsc{Cplex}}
 ...
 We used two different kind of benchmark sets from~\cite{bajuelos2004partitioning,tomas2006visibility}:
     \begin{itemize}
     	 \item \emph{MinArea} instances: include polygons with small areas and tiny interior. They present a lower boundary case for the cardinality of set $D(P)$ which means that, beside the polygon vertices,  $D(P)$ contains few additional points.
	...
	\begin{itemize}
		...
		\end{equation}
    This approach can be justified by the assumption that cameras which cover larger range of points should be of a higher spectrum and quality, and thus more expensive.
...
w_i = \frac{l_i + l_{i+1}}{2}.
 ...
 Similarly as in the case of $W0$, if the arithmetic length of both edges that comes out of vertex $v_i$ is longer, it is expected that a guard can see a larger pieces of polygon $P$. This implies that the spectrum of camera installed at $v_i$ has to be larger, which again implies that it should be of a higher quality, i.e,. a higher price.
	...
	\end{itemize}
     ...
     The benchmark sets and the executable file of our software for this project are provided at public git repository, available on \href{https://github.com/milanagrbic/WOAGP}{https://github.com/milanagrbic/WOAGP}
    ...
    \end{itemize}
     ...
     \begin{table}[]
     	...
     	\begin{tabular}{l|lllllll}    \hline
     		\textit{Algorithm}           & $\overline{obj}$ & $\overline{t}[s]$ &   $\overline{|g|}$ &\#optHit & $\sigma(Opt)$ & \#hitsMinH & $\sigma(MinH)$ \\ \hline
     ...
     \end{table}
      ...
      \begin{table}[]
      	...
      	\end{tabular}}
         \caption{The results on the benchmark set $MinArea$ for the  type of weight $W1$.}\label{small_w1}
      \end{table}
